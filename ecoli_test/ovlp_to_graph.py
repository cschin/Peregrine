...
from falcon_kit.util.ordered_set import OrderedSet as set
...
def generate_string_graph(args):
    ...
    for n in list(G.nodes()):
        ...
        if best_edge is not None:
            ...
            length = G.edges[be0, be1]["length"]
            score = G.edges[be0, be1]["score"]
            subG.add_edge(be0, be1, weight=best_weight, length=length, score=score, identity=100)
...
def ovlp_to_graph(args):
    ...
    for v, w in edge_data:
        try:
            ...
            print( "not in edge_data:", reverse_end(w), reverse_end(v) )
