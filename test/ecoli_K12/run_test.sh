...
time (for c in `seq 1 6`; do echo "/usr/bin/time $SHIMMERBIN/build_shimmer_index -i $INDEX/seq_dataset.idx -t 6 -c $c -o $INDEX/shimmer 2> build_index.$c.log" ; done | parallel -j 4)
echo
echo build overlaps
