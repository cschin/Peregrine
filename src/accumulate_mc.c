...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "p:t:")) != -1) {
		switch (c) {
			...
			case '?':
				if (optopt == 'd') {
					fprintf (stderr, "Option -%c not specified, please specify a prefix of file path the data filis\n", optopt);
				}
				if (optopt == 't') {
					fprintf(stderr, "Option -%c not specified, please specify the total number of chunks\n", optopt);
                }
				return 1;
			default:
				abort();
	...
	for (chunk = 1; chunk <= total_chunk; chunk++) {
		mm_count_v mc = {0, 0, 0};
	    written = snprintf(mc_chunk_file_path, sizeof mc_chunk_file_path, "%s-MC-%02d-of-%02d.dat", data_path_prefix, chunk, total_chunk);
	...
	written = snprintf(mc_file_path, sizeof mc_file_path, "%s-MC-all.dat", data_path_prefix);
